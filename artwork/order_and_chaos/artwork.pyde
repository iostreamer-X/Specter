...
def jitteryLine(start_x, start_y, end_x, end_y, max_height=3, x_step=1, y_step=1, chance_threshold = 40, iteration = 1):
	...
	for scan_x in range(start, end, x_step):
		...
		y = next_y
		x = next_x
		stroke(map(noise(scan_x), 0, 1, 200 - iteration*1.4, 200 ))
...
def jitteryCircle(center_x, center_y, radius, turns = 1, iteration = 1):
	...
	for angle in range(0, (360 * turns) + 1) :
		...
		if x is None or y is None:
			...
			y = next_y
			continue
		max_height = map(noise(angle), 0, 1, 0, 50)
...
for iteration in range(1, 160):
	...
	jitteryCircle(width/2, height/2, radius + map(noise(iteration), 0, 1, radius*iteration*0.7, radius*iteration), 9, iteration=iteration)

# for iteration in range(1, 20):
# 	strokeWeight(map(noise(iteration), 0, 1, 2, 2.1))
# 	jitteryCircle(width/2 - distance, height/2, radius + map(noise(iteration), 0, 1, radius*iteration*0.7, radius*iteration), 9, iteration=iteration*3)
